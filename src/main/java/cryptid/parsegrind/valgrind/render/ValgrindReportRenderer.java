...
public class ValgrindReportRenderer {
    ...
    public Tag render() {
        ...
        return html(
                head(
                        ...
                        link()
                            .withRel("stylesheet")
                            .withHref("https://cdnjs.cloudflare.com/ajax/libs/prism/1.19.0/themes/prism.min.css"),
                        link()
                                .withRel("stylesheet")
                                .withHref("https://cdnjs.cloudflare.com/ajax/libs/prism/1.19.0/plugins/line-numbers/prism-line-numbers.min.css"),
                        link()
                                .withRel("stylesheet")
                                .withHref("https://cdnjs.cloudflare.com/ajax/libs/prism/1.19.0/plugins/line-highlight/prism-line-highlight.min.css"),
                        link()
                            .withRel("stylesheet")
                            .withHref("https://fonts.googleapis.com/css?family=Source+Sans+Pro&display=swap"),
                        style(rawHtml(loadMainStylesheet()))
                ),
                body(
                        div(attrs(".content"),
                            h1("Valgrind Report"),
                            ...
                            script()
                                .withSrc("https://cdnjs.cloudflare.com/ajax/libs/prism/1.19.0/prism.min.js"),
                            script()
                                .withSrc("https://cdnjs.cloudflare.com/ajax/libs/prism/1.19.0/plugins/autoloader/prism-autoloader.min.js"),
                            script()
                                .withSrc("https://cdnjs.cloudflare.com/ajax/libs/prism/1.19.0/plugins/line-numbers/prism-line-numbers.min.js"),
                            script()
                                .withSrc("https://cdnjs.cloudflare.com/ajax/libs/prism/1.19.0/plugins/line-highlight/prism-line-highlight.min.js"),
                            script(rawHtml(loadMainScript()))
                        )
                )
        );
    }
    ...
    private Tag renderPreamble() {
        return section(attrs(".preamble"),
                p(
                        text("Report generated by "),
                        a("parsegrind")
                                .withHref("https://github.com/cryptid-org/parsegrind")
                                .withTarget("_blank"),
                        text(".")
                ));
    }
    ...
    private Tag renderProcessList(final List<ValgrindProcess> processesWithErrors) {
        return section(attrs(".processes"),
                h2("Processes"),
                table(
                        thead(
                                tr(
    ...
    private Tag renderProcess(final ValgrindProcess process) {
        return section(attrs(idForProcess(process) + ".process"),
    ...
    private Tag renderError(final ValgrindProcess process, final ValgrindError error) {
        return div(attrs(idForError(process, error) + ".error"),
                h3(error.getUniqueId() +  " " + error.getKind()),
                div(attrs(".error-description"),
                        p(error.getDescription())
                ),
                div(attrs(".error-stacktrace"),
                        h4("Stacktrace"),
                        div(attrs(".error-stacktrace-frames"),
    ...
    private Tag renderFrame(final ValgrindStacktraceFrame frame) {
        return div(attrs(".error-stacktrace-frame"),
                table(
                    tbody(
                        objectRow(frame),
                        functionRow(frame),
    ...
    private Tag objectRow(final ValgrindStacktraceFrame frame) {
        return tr(
                td(b("Object")),
                td(attrs(".error-stacktrace-frame-object-value"), frame.getObjectName())
        );
    }
    ...
    private Tag functionRow(final ValgrindStacktraceFrame frame) {
        return tr(
                td(b("Function")),
                td(attrs(".error-stacktrace-frame-function-value"), frame.getFunctionName())
        );
    }
    ...
    private Tag fileRow(final ValgrindStacktraceFrame frame) {
        ...
        if (nonNull(frame.getFilePathAndLine()) && frame.getFilePathAndLine().startsWith(configuration.baseDirectory)) {
            ...
            renderPath = text(frame.getFilePathAndLine());
        }

        return tr(
                td(b("File/Line")),
                td(attrs(".error-stacktrace-frame-file-value"), requireNonNullElse(renderPath, text("Unknown location.")))
        );
    }
    ...
    private Tag codeRow(final ValgrindStacktraceFrame frame) {
        ...
        if (isNull(frame.getFilePath()) || !sourcePathFilter.accept(Paths.get(frame.getFilePath()))) {
            code = b("Source code not available or ignored.");
        } else {
        ...
        return tr(
                td(b("Code")),
                td(attrs(".error-stacktrace-frame-code-value"), code)
        );
    }
